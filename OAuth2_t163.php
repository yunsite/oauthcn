...
class T163OAuth {
    ...
    public $host = 'https://api.t.163.com/'; //地址前端
	
    public static $boundary = ''; //静态 多块的边界
    public static $Referer = 'api.t.163.com'; //来源
...
function __construct($app_key, $app_secret) {
    ...
    function access_tokenURL() { return 'https://api.t.163.com/oauth2/access_token'; }
...
function get_user_id($access, $get_arr){
    $user = Array();
  if(empty($access['expires_in'])){
    $user['id'] = NULL;
  }else{
      $response = $this->http($this->host ."account/verify_credentials.". $this->format ."?access_token=". $access['access_token'] , 'GET');
      $user_arr = json_decode($response, true);
	  if(empty($user_arr['error_code'])){
        $user['id'] = $user_arr['id'];
...
function get_oauth_call($url, $method, $params, $pic = null){
  ...
  if (!empty($pic)){
    $params['pic'] = '@'.$pic;
    $pic_json = $this->oAuthRequest('statuses/upload', 'POST', $params, true);
    $pic_url = json_decode($pic_json, true);
	if (!empty($pic_url['upload_image_url'])){
	  $params['status'] = $params['status'].$pic_url['upload_image_url'];
    }
  }
 
  // OAuth的请求包装
  $response = $this->oAuthRequest($url, $method, $params, false);
