...
class TIANYAOAuth {
    ...
    public $host = 'http://open.tianya.cn/api/'; //地址前端
    ...
    public static $Referer = 'open.tianya.cn'; //来源

    public static $pic_name = "media"; //图片名称
/***
  * 构造函数
  */
function __construct($client_id, $client_secret) {
    ...
    // 授权地址
    function request_tokenURL() { return 'http://open.tianya.cn/oauth/request_token.php'; }
    function authorizeURL() { return 'http://open.tianya.cn/oauth/authorize.php'; }
    function access_tokenURL() { return 'http://open.tianya.cn/oauth/access_token.php'; }
...
function get_user_id($access){
  ...
  $info = $this->get_oauth_call('user/info.php', 'POST', array());
  $user['id'] = $info['user']['user_id'];
    return $user;
} //END get_user_id
...
function get_oauth_call($url, $method, $params, $pic = null){
  ...
  $params['appkey'] = $this->client_id; //分配的appkey
  $params['oauth_token'] = $this->access_token; //用户token
  $params['oauth_token_secret'] = $this->token_secret; //用户secret
  $params['timestamp'] = time(); //时间戳
  $params['outformat'] = $this->format; //格式

  strtoupper($method); //把字母转换大小

  //MD5签名值，密钥为：App Secret和Access Token Secret
...
public static function build_http_query_multi($params) {
  ...
  foreach ($params as $parameter => $value) {
    if( in_array($parameter, array('pic', 'image')) && $value{0} == '@' ) {
        ...
        $multipartbody .= 'Content-Disposition: form-data; name="' .self::$pic_name. '"; filename="' . $filename . '"'. "\r\n";
