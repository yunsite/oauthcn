...
class T163OAuth {
    ...
    public $host = 'http://api.t.163.com/'; //地址前端
    ...
    public static $Referer = 't.163.com'; //来源
...
function __construct($client_id, $client_secret) {
    ...
    // 授权地址
    function request_tokenURL() { return 'http://api.t.163.com/oauth/request_token'; }
    ...
    function access_tokenURL() { return 'http://api.t.163.com/oauth/access_token'; }
...
function get_authorizeURL($token, $callback=null, $wicket=0) {
 switch($wicket){
    case '1':
        $wicketURL = '&client_type=web';
        break;
    case '2':
        $wicketURL = '&client_type=mobile';
        break;
    default:
        $wicketURL = '';
 } 
 if(empty($callback)) {
   return $this->authorizeURL() . '?oauth_token='.$token.$wicketURL;
 }else{
   return $this->authenticateURL() . '?oauth_token='.$token.$wicketURL.'&oauth_callback='. urlencode($callback);
 }
...
/***
  ...
  *
  * $oauth_verifier 验证码authorize方式需要
 **/
function get_access_token($oauth_token, $oauth_token_secret, $oauth_verifier=null){
 ...
 if(!empty($callback)) $params['oauth_verifier'] = $oauth_verifier;//验证码
...
function get_user_id($access){
  ...
  $info = $this->get_oauth_call('users/show', 'GET', array()); //'name'=>'', 'user_id'=>''
  $user['id'] = $info['id'];
    return $user;
} //END get_user_id
...
function get_oauth_call($url, $method, $params, $pic = null){
  ...
  $pairs = array_merge($signature ,$params); //合并数组
    if (!empty($pic)){
    $image_url = $this->get_oauth_pic($pic);
    $pairs['status'] .= $image_url['upload_image_url'];
  }
  ...
  // OAuth的请求包装  
  $response = $this->oAuthRequest($url, $method, $pairs, false);
 ...
 **/
function get_oauth_pic($pic){
  $method = 'POST';
  $multi = true;
  $url = $this->host.'statuses/upload.'.$this->format;
  $pairs = $this->oauth_arr_params();
  $pairs['oauth_token'] = $this->access_token; //授权token
  strtoupper($method); //把字母转换大小
