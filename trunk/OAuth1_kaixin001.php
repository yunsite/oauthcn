...
class KAIXIN001OAuth {
    ...
    public $host = 'http://api.kaixin001.com/'; //地址前端
    ...
    public static $Referer = 't.kaixin001.com'; //来源
    
    public $oauth_scope = 3; //用户权限 1用户、2好友、3功能
...
function __construct($client_id, $client_secret) {
    ...
    // 授权地址
    function request_tokenURL() { return 'http://api.kaixin001.com/oauth/request_token'; }
    function authorizeURL() { return 'http://api.kaixin001.com/oauth/authorize'; }
    function access_tokenURL() { return 'http://api.kaixin001.com/oauth/access_token'; }


    // 32位随机码
    function rand32() { return md5(uniqid(rand(),TRUE)); }

    // 默认 oauth 必要配置
    function oauth_arr_params() {
     return array(
       'oauth_consumer_key' => $this->client_id, //App Key
       'oauth_nonce' => $this->rand32(), //32位随机码
...
/***
  * 访问权限设置
  * 0 当前(功能create_records 记录=微博)
  * 1 用户
  * 2 好友
  * 3 功能
  */
function oauth_scope_option($limits = null) {
 $scope = 'basic create_records';
 if($limits >= 1){
   $scope .= ' user_birthday user_bodyform user_blood user_marriage user_intro user_education user_career';
   $scope .= ' user_records user_photo user_messagebox user_rgroup user_repaste user_diary user_forward user_places';
 }
 if($limits >= 2){
   $scope .= ' friends_birthday friends_bodyform friends_blood friends_marriage friends_intro friends_education friends_career';
   $scope .= ' friends_records friends_photo friends_repaste friends_diary friends_places';
 }
 if($limits >= 3){
   $scope .= ' create_album upload_photo send_message create_rgroup create_talk create_repaste create_diary places_checkin';
 }
 return $scope;
} //END oauth_scope_option
...
function get_request_token($callback= null){
 ...
 if(!empty($callback))$params['oauth_callback'] = $callback; //反回地址
 if($this->oauth_scope > 0)$params['scope'] = $this->oauth_scope_option($this->oauth_scope); //权限
...
function get_authorizeURL($token, $callback=null, $wicket=0) {
 $URL = $this->authorizeURL() . '?oauth_token=' . $token;
     return $URL.'&oauth_callback='.$callback;
...
function get_access_token($oauth_token, $oauth_token_secret, $oauth_verifier){
 ...
 $params['oauth_verifier'] = $oauth_verifier; //验证码
...
function get_user_id($access){
  ...
  $uid = explode('_', $user['access_token']);
  if(is_numeric($uid[0])){
    $user['id'] = $uid[0];
  }else{
    $this->access_token = $user['access_token']; //用户授权token
    $this->token_secret = $user['access_token_secret']; //用户授权token
    $info = $this->get_oauth_call('users/me', 'GET', array());
    $user['id'] = $info['uid'];
  }
    return $user;
} //END get_user_id
