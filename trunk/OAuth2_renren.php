...
class RENRENOAuth {
    ...
    public $host = 'http://api.renren.com/restserver.do'; //地址前端
	
    public static $boundary = ''; //静态 多块的边界
    public static $Referer = 'api.renren.com'; //来源
...
function __construct($app_key, $app_secret) {
    ...
    // 授权地址
    function authorizeURL() { return 'https://graph.renren.com/oauth/authorize'; }
    function access_tokenURL() { return 'https://graph.renren.com/oauth/token'; }
 ...
 **/
function get_authorizeURL($uri, $type, $wicket = NULL, $state = '2') {
   $state_string = '';
  if($state == '0' || $state == '1' || $state == '2'){
   //获取信息类权限
   $state_string .=
   'read_user_blog read_user_checkin read_user_feed read_user_guestbook read_user_invitation '.
   'read_user_like_history read_user_message read_user_notification read_user_photo '.
   'read_user_status read_user_album read_user_comment read_user_share read_user_request';
  }
  if($state == '1' || $state == '2'){
   //发布信息类权限
   $state_string .=
   ' publish_blog publish_checkin publish_feed publish_share write_guestbook send_invitation send_request '.
   'send_message send_notification photo_upload status_update create_album publish_comment operate_like';
  }
  if($state == '2'){
   //其他权限
   $state_string .= ' admin_page';
   }
   $params = array(
       'client_id' => $this->app_key,//App Key
       'redirect_uri' => $uri,//授权回调地址
       'response_type' => $type,//返回类型，支持code、token，默认值为code
       'state' => $this->URLencode_RFC($state_string) //用于保持请求和回调的状态
    );
/** 2012-7-4 23:56
 switch($wicket){
    case '1':
        $params['display'] = 'popup'; //弹窗小窗口
        break;
    default:
        $params['display'] = 'default'; //web浏览器
 }
 **/
...
function get_access_token($type, $arr) {
 ...
 $get_params = $this->oAuthRequest($url, 'POST', $params);
     return json_decode($get_params, true);
...
function get_oauth_call($url, $method, $params, $pic = null){
  $params['v'] = '1.0'; //API的版本号
  $params['type'] = '20'; //分享的类型：日志为1、照片为2、链接为6、相册为8、视频为10、音频为11、分享为20
  $params['format'] = $this->format; //请指定为JSON或者XML
  $params['method'] = $url; //接口名
    
  $params['access_token'] = $this->access_token; //授权token
  
  $signature['sig'] = $this->get_signature($url, $params['call_id'], $params['session_key']); //签名认证
  ...
  // OAuth的请求包装  
  $response = $this->oAuthRequest($this->host, $method, $params, $multi);
...
/**
  ...
  * 返回string
  */
function get_signature($url, $call_id, $session_key){
    $params = array();
	$params['v'] = '1.0';
 ...
 $base_string = MD5($pairs.$this->app_secret);
   return $base_string;
} //END get_signature
