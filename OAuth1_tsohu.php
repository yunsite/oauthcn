...
class TSOHUOAuth {
    ...
    public $host = 'http://api.t.sohu.com/'; //地址前端
    ...
    public static $Referer = 'api.t.sohu.com'; //来源
...
function __construct($client_id, $client_secret) {
    ...
    // 授权地址
    function request_tokenURL() { return 'http://api.t.sohu.com/oauth/request_token'; }
    function authorizeURL() { return 'http://api.t.sohu.com/oauth/authorize'; }
    function access_tokenURL() { return 'http://api.t.sohu.com/oauth/access_token'; }
...
function get_authorizeURL($token, $callback=null, $wicket=0) {
 ...
 if (!empty($callback)) return  $url.'&oauth_callback='.$this->URLencode_RFC($callback); //反回地址
   else return $url;
...
function get_user_id($access){
  ...
  $info = $this->get_oauth_call('users/show', 'GET', array());
  $user['id'] = $info['id'];
    return $user;
} //END get_user_id
...
function get_oauth_call($url, $method, $params, $pic = null){
  ...
  //$url = "http://weatherrobot.sinaapp.com/to.php";
  ...
  if (!empty($pairs['status'])) $pairs['status'] = $this->URLencode_RFC($pairs['status']); //内容编码
...
function oAuthRequest($url, $method, $parameters, $multi = false) {
  ...
  switch ($method) {
    ...
    default:
     ...
     return $this->http($url, $method, $body, $headers, $multi = false);
  }
}
 ...
 * @ignore
 */
function http($url, $method, $postfields = NULL, $headers = array(), $multi = false) {
...
// 模拟客户端发送文件头
  //$headers[] = "Authorization:OAuth oauth_consumer_key=" . $this->client_id;
  ...
  curl_setopt($ci, CURLINFO_HEADER_OUT, TRUE );//追踪句柄的请求字符串
