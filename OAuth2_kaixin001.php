...
class KAIXIN001OAuth {
    ...
    public $host = 'https://api.kaixin001.com/'; //地址前端
	
    public static $boundary = ''; //静态 多块的边界
    public static $Referer = 'api.kaixin001.com'; //来源
...
function __construct($app_key, $app_secret) {
    ...
    // 授权地址
    function authorizeURL() { return 'http://api.kaixin001.com/oauth2/authorize'; }
    function access_tokenURL() { return 'https://api.kaixin001.com/oauth2/access_token'; }
 ...
 **/
function get_authorizeURL($uri, $type, $wicket = NULL, $state = NULL) {
   $params = array(
       'client_id' => $this->app_key,//App Key
       'redirect_uri' => $uri,//授权回调地址
       'response_type' => $type,//返回类型，支持code、token，默认值为code
       'scope' => 'basic create_records',//设置权限，支持code、token，默认的basic权限
 ...
 switch($wicket){
    case '1':
        ...
        break;
    case '2':
        $params['display'] = 'mobile'; //移动终端
        break;
    case '3':
        $params['display'] = 'wap'; //wap版授权页面，适用于非智能手机
		break;
    case '4':
        $params['display'] = 'js'; //微博JS-SDK专用授权页面，弹窗类型，返回结果为JSONP回掉函数
    case '5':
        $params['display'] = 'client'; //客户端版本授权页面，适用于PC桌面应用
		break;
    case '6':
        $params['display'] = 'apponweibo'; //默认的站内应用授权页，授权后不返回access_token，只刷新站内应用父框架
		break;
    default:
...
function get_user_id($access, $get_arr){
  ...
  if(empty($access['access_token'])){	
	...
	$access_token = explode('_', $access['access_token']);
	$user['id'] = $access_token[0];
